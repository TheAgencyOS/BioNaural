...
private struct HomeTab: View {
    ...
    // User's saved compositions — drives the Home-screen Quick Play
    // rail. If empty, the rail is hidden. Each tap navigates into
    // the existing composedSession flow.
    @Query(sort: \CustomComposition.createdDate, order: .reverse)
    private var savedCompositions: [CustomComposition]
    ...
    // MARK: - Quick Play (user's saved compositions)

    /// Horizontal scroll of the user's saved compositions. Each tap
    /// launches the session for that composition via the existing
    /// `.composedSession(id:)` navigation route. Hidden entirely
    /// when the user has no saved compositions — we used to show
    /// catalog-based tracks here but those led to generic sessions
    /// that didn't actually play what was labeled.
    @ViewBuilder
    private var recommendedTracksSection: some View {
        if !savedCompositions.isEmpty {
            VStack(alignment: .leading, spacing: Theme.Spacing.sm) {
                Text("QUICK PLAY")
                    .font(Theme.Typography.caption)
                    .tracking(1.5)
                ...
                ScrollView(.horizontal, showsIndicators: false) {
                    HStack(spacing: Theme.Spacing.md) {
                        ForEach(savedCompositions.prefix(8)) { composition in
                            NavigationLink(value: AppDestination.composedSession(id: composition.id)) {
                                savedCompositionCard(composition)
                            }
                            .buttonStyle(.plain)
                        }
                    }
    ...
    private func savedCompositionCard(_ composition: CustomComposition) -> some View {
        let mode = FocusMode(rawValue: composition.brainState) ?? .focus
